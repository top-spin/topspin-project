<template>
  <form class="dropzone">
    ...
    <div id="dropzone-message" style="display: none">
      ...
      <span class="dropzone-info">Upload a profile picture here</span>
    </div>
  </form>
</template>
...
export default {
  ...
  mounted () {
    ...
    let options = {
      ...
      accept (file, done) {
        lambda.getSignedURL(file)
          .then((url) => {
            file.uploadURL = url
            console.log("file from line 58",file)
            console.log("url from line 59",url)
            done()
    ...
    vm.dropzone.on('processing', (file) => {
      this.$store.commit('SET_AVATAR',("https://s3.amazonaws.com/devmountaintopspin/"+file.upload.filename))
